...
function getFactorial(n) {
  let toReturn = 1;
  for (let i = 2; i <= n; i += 1) toReturn *= i;
  return toReturn;
}
...
function reverseInteger(num) {
  const f = num.toString().split('');
  return Number(f.reverse().join(''));
}
...
function isCreditCardNumber(cnn) {
  let sum = 0;
  const cnnArr = cnn.toString().split('');
  for (let i = 0; i < cnnArr.length; i += 1) {
    let cardNum = parseInt(cnnArr[i], 10);

    if ((cnnArr.length - i) % 2 === 0) {
      cardNum *= 2;
